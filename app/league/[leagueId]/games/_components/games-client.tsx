            ...
            groupedGames.map(([round, games]) => (
              <section key={round} className="space-y-3">
                <div className="flex items-center gap-2">
                  <h2 className="text-lg font-semibold text-white">{formatRound(round)}</h2>
                  <span className="rounded-full border border-neutral-700 bg-neutral-800 px-2.5 py-0.5 text-xs text-neutral-400">
                    {games.length} games
                  </span>
                </div>
                <div className="space-y-3">
                  {games.map((game) => {
                    ...
                    return (
                      <article
                        key={game.id}
                        className="rounded-2xl border border-white/10 bg-neutral-900/95 p-4 shadow-lg shadow-black/20"
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {upset ? (
                              <span className="inline-flex items-center gap-1 rounded-full bg-violet-500/15 px-2.5 py-1 text-[11px] font-medium text-violet-200">
