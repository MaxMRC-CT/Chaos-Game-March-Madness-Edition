...
/** POST /api/dev/apply-round-replay - Apply round(s) from data/2025/results.json */
export async function POST(request: NextRequest) {
  ...
  if (!fs.existsSync(RESULTS_PATH)) {
    return NextResponse.json(
      { ok: false, error: `results.json not found at data/2025/results.json` },
      { status: 500 },
    );
  }
  ...
  if (action === "reset") {
    ...
    await prisma.leagueScoreSnapshot.deleteMany({ where: { leagueId: league.id } });
  ...
  if (action === "replay_full") {
    ...
    const teamByNormShort = new Map(
      teams.filter((t) => t.shortName).map((t) => [normalize(t.shortName!), t.id]),
    );
    ...
    for (const roundArg of ROUNDS) {
      const roundKey = roundArg === "NCG" ? (data["NCG"] ? "NCG" : "FINAL") : roundArg;
      const roundGames = data[roundKey];
      if (!roundGames || !Array.isArray(roundGames) || roundGames.length === 0) continue;
    ...
    return NextResponse.json({
      ...
      action: "replay_full",
      applied,
      message: `Replayed full tournament: ${applied.length} rounds applied.`,
    });
  }
